...
#* ##############Part Two 第二部分 表单类和基础类创建 基础用户方法实现#################
#* ################################################################################
#* ################################################################################
#* ################################################################################
#* ################################################################################
...
class article_input(baseform):
    ...
    textinput=TextAreaField("write your article here:",validators=[Length(20,32767)])
    submit=SubmitField("Confirm")
...
class user_base():
    ...
    def write_to_sql(self):
        ...
        #* 将用户信息写入数据库
        cursor.execute("insert into users (userid,password,description,email,confirm_code,if_admin,zhuce_time,if_vip,p2times) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)",(self.name,self.word,self.desc,self.emai,int(self.code),self.admi,self.time,self.vipa,self.shen))
        connect.commit()
...
def logst(from_,body,level=1):
    ...
    time_=get_time()
    try:
        with open("O:\\ETRO.web\\project\\logging\\loggings-%s.log"%(time_[0:10].replace("/","-")),"a",encoding="utf8") as loga:
            loga.write("[%s]-%s-------FROM:%s%sBODY:%s\n"%(time_,type_,from_,lj,body))
    except:
        with open("O:\\ETRO.web\\project\\logging\\loggings-%s.txt"%(time_[0:10].replace("/","-")),"w",encoding="utf8") as loga:
            loga.write("[%s]-%s-------FROM:%s%sBODY:%s\n"%(time_,type_,from_,lj,body))
        os.rename("O:\\ETRO.web\\project\\logging\\loggings-%s.txt"%(time_[0:10].replace("/","-")),r"logging/loggings-%s.log"%(time_[0:10]))
