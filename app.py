...
def articles():
    if "logged-in" in session:
        cursor.execute("select head from article where level<>2")
...
def article():
    ...
    if headers!="social":
        cursor.execute("select body_place from article where head=%s and level<>2",(head))
        answer=cursor.fetchall()
        if len(answer)==0:
...
admincodes=[
    "addsdaqehwdiudyqoiuyr21e1y8jldhajldskhuoqeu"
]
...
@app.route("/social/view")
def view_a_social():
    if "to_up2" in session or "logged_in" in session:
        social_code=request.args.get("code")
        cursor.execute("select owner,name,state,description from social where member_place=%s",(social_code))
        ...
        if state==1: 
            ...
            seeable=True
        cursor.execute("select head from %s_article")
