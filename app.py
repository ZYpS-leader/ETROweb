...
handler = RotatingFileHandler('app.log', maxBytes=100*1048576, backupCount=10)
handler.setLevel(INFO)
fromt=Formatter("%(asctime)s-%(name)s-%(levelname)s-%(message)s")
handler.setFormatter(fromt)
"""
...
vipcodes=[
    "1145141919810_djdhjskdjh",
    "shdajdofhfasoifhsjlajkdh"
]
#* 创建新账户(输入信息界面)
...
def create():
    ...
    if form.validate_on_submit():
        if "logged-in" in session:
            ...
            text_inputd=form.textinput.data

            article=an_article(text_head,text_inputd,username,get_time())
            article.in_to_sql()
...
def article():
    ...
    cursor.execute("select body_place,author,_time from article where head=\"%s\""%head)
    answer=cursor.fetchall()
    if len(answer)==0:
    ...
    _time=result[2]
    maint=""
    with open("files/txt/%s.txt"%filepath,"r",encoding="utf8") as reads:
        text=reads.readlines()
        for i in text:
            if i=="\n":continue
            else:maint+=i.replace("\n","<br>")
    return render_template("article.html",head=head.replace("_"," "),author=author,time=_time,m=maint)
...
@app.route("/admin/deluser",methods=["GET","POST"])
def deluser():
    form=del_user()
...
@app.route("/admin/delarti",methods=["GET","POST"])
def delarti():
    form=del_arti()
    if form.validate_on_submit() and session["admin"]:
        ...
        flash("Delete!")
        return redirect(url_for("admin"))
    return render_template("admin_del_arti.html",form=form)
...
def user_space():
    if "logged-in" in session:
        ...
        for i in range(0,len(ARTIS1)):
            ...
            b=ARTIS2[i]
            reponse.append({"xhx":a,"ykg":b})
        cursor.execute("select zhuce_time,description,if_vip from users where `userid`='%s'"%est(username))
        ...
        return render_template("auser.html",ARTICLES=reponse,a_user=a_user)
    return redirect("/signin")
...
def patch():
    if "logged-in" in session:
        return render_template("patch_main.html")
    return redirect("/signin")

@app.route("/patch/text",methods=['GET','POST'])
def patch_1():
    if "logged-in" in session:
        form=_patch_1()
        if form.validate_on_submit():
...
@app.route("/patch/image",methods=["GET","POST"])
def patch_2():
    if "logged-in" in session:
        form=_patch_2()
        if form.validate_on_submit():
...
@app.route("/patch/get_token",methods=["GET","POST"])
def patch_3():
    if "logged-in" in session:
        form=_patch_3()
        if form.validate_on_submit():
...
@app.route("/havevip",methods=["GET","POST"])
def havevip():
    form=getvip()
    try:
        cursor.execute("select if_vip from users where userid=%s",(session["n"]))
    except KeyError:
        return redirect(url_for("signin"))
    get=cursor.fetchall()
    ifvip=get[0]
